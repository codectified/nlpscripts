...
# Step 1: Parse the Arabic poem text file
def parse_poem_text(file_path):
    words_data = []
    with open(file_path, 'r', encoding='utf-8') as f:
        for line_number, line in enumerate(f, start=1):
            ...
            for word_position, word in enumerate(words, start=1):
                # Create a location string with line and word position for each word
                location = f"{line_number}:{word_position}"
                words_data.append({
                    "word": word,
...
def process_batch_results(output_file_name="batch_output.jsonl", words_data=None, append=False):
    ...
    with open(output_file_name, 'r', encoding='utf-8') as f:
        for line in f:
            ...
            csv_output.append([
                ...
                word_data['line_number'],    # Line number (replaces sura index)
                word_data['word_position'],  # Word position in line (replaces aya index)
    ...
    with open('poem_output_for_graphdb.csv', mode, newline='', encoding='utf-8') as csvfile:
        fieldnames = ['word', 'line_number', 'word_position', 'classification']
...
def main():
    ...
    if choice == "1":
        ...
        words_data = parse_poem_text("ashanfara.txt")
        
        # Adjust this to handle all batch files
        num_batches = 16  # Adjust number based on batch files
        ...
        # Set batch size
        batch_size = 1000  # Adjust based on needs
        ...
        for start_idx in range(0, total_words, batch_size):
            ...
            append = start_idx != 0  # Append after the first batch
