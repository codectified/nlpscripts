...
class SemiticWordsIntegrator:
    def __init__(self):
        # Rich console setup first (needed for logging)
        if RICH_AVAILABLE:
    ...
    def setup_logging(self):
        ...
        self.logger.info(f"🎨 Rich available: {RICH_AVAILABLE}, Console: {self.console is not None}")
    ...
    def check_word_duplicate(self, word: str, lang: int, sem_id: int) -> Dict[str, any]:
        """
        Check if word already exists and return status info.
        
        Returns:
            Dict with 'is_duplicate', 'needs_update', 'element_id', 'has_sem_lang'
        """
        try:
            with self.driver.session() as session:
                query = """
                ...
                RETURN elementId(w) as element_id, w.sem_lang as current_sem_lang
                """
                result = session.run(query, word=word, lang=lang, sem_id=sem_id)
                record = result.single()
                
                if record:
                    current_sem_lang = record.get('current_sem_lang')
                    has_sem_lang = current_sem_lang is not None
                    needs_update = not has_sem_lang
                    
                    self.logger.debug(f"🔍 Duplicate detected: '{word}' (lang: {lang}, sem_id: {sem_id}) | has_sem_lang: {has_sem_lang}")
                    
                    return {
                        'is_duplicate': True,
                        'needs_update': needs_update,
                        'element_id': record['element_id'],
                        'has_sem_lang': has_sem_lang,
                        'current_sem_lang': current_sem_lang
                    }
                else:
                    return {
                        'is_duplicate': False,
    ...
    def update_word_with_sem_lang(self, element_id: str, sem_lang: str, word: str) -> bool:
        """
        Update existing Word node with sem_lang property.
        ...
        try:
            with self.driver.session() as session:
                query = """
                MATCH (w:Word) 
                WHERE elementId(w) = $element_id
                SET w.sem_lang = $sem_lang
                RETURN w.word as updated_word
                """
                result = session.run(query, element_id=element_id, sem_lang=sem_lang)
                record = result.single()
                
                if record:
                    self.logger.info(f"🔄 Updated word: '{word}' with sem_lang: '{sem_lang}'")
                    if RICH_AVAILABLE and self.console:
                        self.console.print(f"[blue]🔄[/blue] [cyan]{word}[/cyan] updated with [dim]{sem_lang}[/dim]")
                    
                    self.stats['duplicates_updated'] = self.stats.get('duplicates_updated', 0) + 1
                    # Throttling
                    time.sleep(self.delay_between_operations)
                    return True
                else:
                    self.logger.warning(f"⚠️ Failed to update word: {word}")
                    return False
                    
        except Exception as e:
            self.logger.error(f"❌ Failed to update word {word} with sem_lang: {e}")
            return False
    ...
    def create_word_and_link(self, word_data: Dict, sem_id: int, sem_word_id: int, create_missing_roots: bool = True) -> bool:
        ...
        try:
            ...
            lang_id = int(word_data.get("lang", 0))
            lang_names = {
                1: 'Arabic', 2: 'Hebrew', 3: 'Sabaic', 4: 'Ugaritic', 5: 'Aramaic',
                6: 'Amharic', 7: 'Phoenician', 8: 'Syriac', 9: 'Geez', 10: 'Akkadian',
                11: 'Qatabanic', 12: 'Eblaite', 13: 'Mehri', 14: 'Safaitic', 15: 'Moabite',
                16: 'Palmyrene', 17: 'Mandaic', 18: 'Hismaic', 19: 'Nabataean', 20: 'Taymanitic',
                21: 'Dadanitic', 22: 'Hasaitic', 23: 'Hadramitic', 24: 'Minaic', 25: 'Amorite',
                26: 'Thamudic_B', 27: 'Harsusi', 29: 'Maltese', 30: 'Ammonite', 31: 'Hatran'
            }
            sem_lang = lang_names.get(lang_id, f"Language_{lang_id}")
            ...
            with self.driver.session() as session:
                ...
                if record:
                    ...
                    self.logger.info(f"✨ Created word: '{created_word}' ({sem_lang}) → root {root_arabic} | sem_id: {sem_id}")
                    
                    if RICH_AVAILABLE and self.console:
                        self.console.print(f"[green]✨[/green] [cyan]{created_word}[/cyan] ([dim]{sem_lang}[/dim]) → [yellow]{root_arabic}[/yellow]")
        ...
        try:
            ...
            for idx, (row_id, row) in enumerate(rows_by_id.items(), start=1):
                ...
                try:
                    ...
                    duplicate_status = self.check_word_duplicate(word_text, lang, sem_id)
                    
                    if duplicate_status['is_duplicate']:
                        if duplicate_status['needs_update']:
                            # Update existing word with sem_lang
                            lang_id = int(row.get("lang", 0))
                            lang_names = {
                                1: 'Arabic', 2: 'Hebrew', 3: 'Sabaic', 4: 'Ugaritic', 5: 'Aramaic',
                                6: 'Amharic', 7: 'Phoenician', 8: 'Syriac', 9: 'Geez', 10: 'Akkadian',
                                11: 'Qatabanic', 12: 'Eblaite', 13: 'Mehri', 14: 'Safaitic', 15: 'Moabite',
                            ...
                            success = self.update_word_with_sem_lang(
                                duplicate_status['element_id'], 
                                sem_lang, 
                                word_text
                            )
                            ...
                            # Already has sem_lang, truly skip
                            self.logger.info(f"⚪ SKIP complete: '{word_text}' already has sem_lang '{duplicate_status['current_sem_lang']}'")
                            if RICH_AVAILABLE and self.console:
                                self.console.print(f"[yellow]⚪ SKIP[/yellow] [dim]'{word_text}' (complete)[/dim]")
                    ...
                    # Update progress
                    if RICH_AVAILABLE and hasattr(self, 'progress') and self.progress:
                    ...
                    if self.stats['total_processed'] % batch_size == 0:
                        ...
                        if RICH_AVAILABLE and self.console:
                            self.console.print(f"[blue]⏳[/blue] Batch pause ({self.delay_between_batches}s) | [green]{self.stats['words_created']} created[/green] | [blue]{self.stats['duplicates_updated']} updated[/blue] | [yellow]{self.stats['duplicates_skipped']} skipped[/yellow]")
    ...
    def print_statistics(self):
        ...
        self.logger.info(f"🔄 Duplicates updated: {self.stats['duplicates_updated']}")
        ...
        if RICH_AVAILABLE and self.console:
            ...
            table.add_row("🔄 Duplicates Updated", str(self.stats['duplicates_updated']), "Existing words updated with sem_lang")
            table.add_row("⚪ Duplicates Skipped", str(self.stats['duplicates_skipped']), "Words already complete")
...
def main():
    ...
    try:
        ...
        if RICH_AVAILABLE and integrator.console:
